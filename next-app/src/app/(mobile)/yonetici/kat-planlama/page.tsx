...
interface RoutePoint { id: string; name: string; point_order: number; detail: string | null; qr_token: string | null }
interface Schedule {
  id: string;
...
export default function KatPlanlama() {
  const router = useRouter();
  const { personnel } = useAuth();

  const [locations, setLocations]   = useState<Location[]>([]);
  const [routes, setRoutes]         = useState<ControlRoute[]>([]);
  ...
  async function loadData() {
    const [locRes, routeRes] = await Promise.all([
      ...
      supabase.from("patrol_routes").select(`
        ...
        points:patrol_route_points(id, name, point_order, detail, qr_token),
  ...
  async function createRoute() {
    ...
    if (!error && data) {
      const nr: ControlRoute = { ...data, points: [], schedules: [] };
      setRoutes(p => [nr, ...p]);
      setExpandedId(nr.id);
  ...
  async function addPoint(routeId: string) {
    ...
    const { data, error } = await supabase.from("patrol_route_points")
      .insert({ route_id: routeId, name: newPointName.trim(), detail: newPointDetail.trim() || null, qr_token, point_order: (route?.points.length ?? 0) + 1 })
      .select("id, name, point_order, detail, qr_token").single();
    if (!error && data) {
      setRoutes(p => p.map(r => r.id === routeId ? { ...r, points: [...r.points, data] } : r));
      setNewPointName(""); setNewPointDetail(""); setAddingPointTo(null);
  ...
  function printQr() {
    ...
    if (!win) return;
    win.document.write(`
      <html><head><title>${qrModal.name}</title></head>
      <body style="text-align:center;font-family:sans-serif;padding:32px;">
        <h2 style="margin-bottom:4px;">${qrModal.name}</h2>
        ${qrModal.detail ? `<p style="color:#666;margin-top:0;">${qrModal.detail}</p>` : ""}
  ...
  function downloadQr() {
    ...
    a.href = qrDataUrl;
    a.download = `qr-${qrModal.name.replace(/\s+/g, "-").toLowerCase()}.png`;
    a.click();
  }
    ...
    <div className="bg-[#f0f2ff] min-h-screen pb-32">
      ...
      <header className="sticky top-0 z-40 flex items-center gap-3 px-4 h-16 shadow-sm"
        ...
        <div className="flex-1">
          <h1 className="font-bold text-white text-lg leading-tight">Kat Kontrolü Planlaması</h1>
        ...
        {filtered.length === 0 ? (
          <div className="bg-white rounded-2xl p-10 flex flex-col items-center gap-4 shadow-sm mt-2">
            <div className="w-16 h-16 bg-indigo-50 rounded-2xl flex items-center justify-center">
              <span className="material-symbols-outlined text-[#3949AB] text-[36px]">cleaning_services</span>
            </div>
            ...
            <div key={route.id} className="bg-white rounded-2xl shadow-sm overflow-hidden">
              ...
              <button onClick={() => setExpandedId(isOpen ? null : route.id)}
                ...
                <div className={`w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0 ${route.is_active ? "bg-teal-100" : "bg-gray-100"}`}>
                  <span className={`material-symbols-outlined text-[22px] ${route.is_active ? "text-teal-600" : "text-gray-400"}`} style={{ fontVariationSettings: "'FILL' 1" }}>cleaning_services</span>
                </div>
                ...
                <div className="border-t border-gray-100">
                  ...
                  <div className="px-4 pt-4 pb-3">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <button onClick={() => { setAddingPointTo(route.id); setNewPointName(""); setNewPointDetail(""); }}
                          ...
                          {route.points.map(pt => (
                            <div key={pt.id} className="flex items-center gap-3 bg-gray-50 rounded-xl px-3 py-3">
                              ...
                              <div className="flex-1 min-w-0">
                                <span className="block text-sm font-semibold text-gray-700 truncate">{pt.name}</span>
                                {pt.detail && <span className="block text-xs text-gray-400 truncate">{pt.detail}</span>}
                              </div>
                              <button onClick={() => setQrModal(pt)}
                                className="w-8 h-8 rounded-full bg-indigo-50 flex items-center justify-center active:scale-90 transition-all flex-shrink-0"
                                title="QR Kodu Görüntüle">
                                <span className="material-symbols-outlined text-[#3949AB] text-[16px]">qr_code_2</span>
                              </button>
                    ...
                    {addingPointTo === route.id && (
                      <div className="mt-3 space-y-2">
                        <input autoFocus value={newPointName}
                          ...
                          placeholder="Nokta adı (örn: 10. Kat WC)"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-6">
          ...
          <div className="relative w-full max-w-[340px] bg-white rounded-3xl shadow-2xl p-6 space-y-4 text-center">
            ...
            <div>
              ...
              {qrModal.detail && <p className="text-xs text-gray-400 mt-0.5">{qrModal.detail}</p>}
            </div>
            ...
            <p className="text-xs text-gray-400">Bu QR kodu yazdırıp lokasyona yapıştırın. Personel işini bitirince bu kodu okutarak kontrolü teyit eder.</p>
            <div className="flex gap-2">
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center">
          ...
          <div className="relative w-full max-w-[430px] bg-white rounded-t-3xl shadow-2xl">
            <div className="px-6 pt-5 pb-4 space-y-4 max-h-[85vh] overflow-y-auto">
              ...
              {/* Aralık */}
              <div>
                <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider block mb-2">Kontrol Aralığı</label>
          ...
          <div className="relative w-full max-w-[430px] bg-white rounded-t-3xl px-6 pt-5 pb-10 shadow-2xl space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <input autoFocus value={newRouteName} onChange={e => setNewRouteName(e.target.value)}
                placeholder="Örn: Genel Müdürlük Katları"
